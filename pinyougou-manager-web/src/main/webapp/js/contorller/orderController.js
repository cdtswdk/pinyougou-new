...
app.controller("orderController", function ($scope, $http, $controller, orderService) {
    ...
    $scope.getPage = function (page, size) {
        //发送请求获取数据
        orderService.findAll(page, size, $scope.searchEntity).success(function (response) {
            //集合数据
    ...
    $scope.save = function () {
        ...
        if ($scope.entity.id != null) {
            //执行修改数据
            result = orderService.update($scope.entity);
        } else {
            //增加操作
            result = orderService.add($scope.entity);
        }
        //判断操作流程
